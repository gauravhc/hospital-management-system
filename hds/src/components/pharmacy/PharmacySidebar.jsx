...
export default function PharmacySidebar({ username, isOpen, onClose }) {
  ...
  return (
    <>
      {isOpen ? <div className="fixed inset-0 z-20 bg-black/40 md:hidden" onClick={onClose} /> : null}
      ...
      >
        <div className="border-b border-slate-200 px-6 py-5">
          <h2 className="text-xl font-semibold text-sky-700">PHARMACY</h2>
          {username ? (
          ...
          {NAV_ITEMS.map((item) => (
            <Link
              ...
              className={`block rounded-xl px-4 py-2.5 text-sm font-medium transition ${
                isActive(item.href) ? "bg-sky-100 text-sky-800" : "text-slate-700 hover:bg-sky-50"
              }`}
            >
              {item.name}
            </Link>
