    ...
    return (
        <div className="md:hidden flex flex-col bg-white dark:bg-slate-900 shadow-sm z-10 w-full">
            ...
            <div className="px-4 py-3 flex justify-between items-center bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                ...
                {/* Notification Bell */}
                <div className="shrink-0 p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-600 dark:text-slate-200">
            ...
            {/* Horizontal Tabs */}
            <div className="flex gap-2 overflow-x-auto max-w-full px-4 pb-0 no-scrollbar border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.href}
                            href={item.href}
                            className={`whitespace-nowrap pb-3 px-2 text-[13px] sm:text-sm font-medium border-b-2 transition-colors ${active
