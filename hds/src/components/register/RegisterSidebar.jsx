...
import { X } from "lucide-react"; // Start using lucide-react as requested in other files, or just text X if lib missing
...
export default function RegisterSidebar({ username, isOpen, onClose }) {
    ...
    const navItems = [
        ...
        { name: "Notifications", href: "/register/notifications" }, // Added Notification
    ];
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    {navItems.map((item) => (
                        <Link
                            ...
                            className={`block px-4 py-2 rounded-md font-medium transition-colors ${isActive(item.href)
