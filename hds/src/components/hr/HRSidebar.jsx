...
export default function HRSidebar({ username, isOpen, onClose }) {
    ...
    <>
      ...
      >
        <div className="border-b border-slate-200 px-6 py-5">
          <h2 className="text-xl font-semibold text-sky-700">HR MANAGER</h2>
          {username ? (
            ...
            return (
              <Link
                ...
                className={`block rounded-xl px-4 py-2.5 text-sm font-medium transition ${
                  active ? "bg-sky-100 text-sky-800" : "text-slate-700 hover:bg-sky-50"
                }`}
              >
                {item.name}
              </Link>
